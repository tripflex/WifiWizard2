  ...
  iOSConnectNetwork: function(ssid, ssidPassword) {
    return new Promise(function(resolve, reject) {
      if (
        ...
        !ssidPassword ||
        ssidPassword.length < 1
      ) {
        // iOS connect open network
        cordova.exec(resolve, reject, "WifiWizard2", "iOSConnectOpenNetwork", [
          { Ssid: ssid }
        ]);
      } else if (
        ssidPassword !== undefined &&
        ssidPassword.length > 0 &&
        ssidPassword.length < 8
      ) {
        // iOS pass length does not meet requirements (min 8 chars for WPA/WPA2)
        reject(
          "WPA/WPA2 password length must be at least 8 characters in length!"
        );
      } else {
